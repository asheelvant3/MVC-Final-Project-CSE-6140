...
def fast_vc(graph, filename, cutoff_time, random_seed):
    ...
    base = filename.split('/')[-1].split('.')[0] \
           ...
           + str(random_seed)
    calc=calculation(start_time,cutoff_time,graph,vertex_cover,base,gain,loss,edges)
...
def run(filename, cutoff_time, random_seed):
    seed(random_seed)
    graph,n = construct_graph(filename)
    fast_vc(graph, filename, cutoff_time, random_seed)
def calculation(start_time,cutoff_time,graph,vertex_cover,base,gain,loss,edges):
    inf = float('inf')
